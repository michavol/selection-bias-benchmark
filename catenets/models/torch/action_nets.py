...
class BasicDragonNet(BaseCATEEstimator):
    ...
    def __init__(
        ...
        self.low_avg_outcome = None
        self.high_avg_outcome = None
    ...
    def loss(
        ...
        return (
            prop_loss(t_pred, t_true)  #+ po_loss(po_pred, y_true, t_true)
        )
    ...
    def train(
        ...
        avg_treated_outcome = torch.mean(y[w == 1])
        avg_control_outcome = torch.mean(y[w == 0])
        self.low_avg_outcome = min(avg_treated_outcome, avg_control_outcome)
        self.high_avg_outcome = max(avg_treated_outcome, avg_control_outcome)
    ...
    def predict(self, X: torch.Tensor, return_po: bool = False, training: bool = False) -> torch.Tensor:
        ...
        if not training:
            ...
            self._propensity_estimator.model.eval()
        ...
        # If prop is larger 0.5 predict one predict y1 with high average outcome else predict y0 with low average outcome
        
        t_preds = torch.argmax(prop_preds, dim=1)
  
        # Write the previous two lines but for torch
        # y0_preds = torch.where(t_preds == 0, torch.tensor(self.high_avg_outcome).to(DEVICE), torch.tensor(self.low_avg_outcome).to(DEVICE))
        # y1_preds = torch.where(t_preds == 1, torch.tensor(self.high_avg_outcome).to(DEVICE), torch.tensor(self.low_avg_outcome).to(DEVICE))
        y0_preds = torch.where(t_preds == 0, torch.tensor(1).to(DEVICE), torch.tensor(0).to(DEVICE))
        y1_preds = torch.where(t_preds == 1, torch.tensor(1).to(DEVICE), torch.tensor(0).to(DEVICE))

        # Fill y0_preds with ones if t_preds is 0 and y1_preds with ones if t_preds is 1
        # y0_preds = torch.where(t_preds == 0, torch.tensor(1).to(DEVICE), torch.tensor(1).to(DEVICE))
        # y1_preds = torch.where(t_preds == 1, torch.tensor(1).to(DEVICE), torch.tensor(0).to(DEVICE))
...
class ActionNet(BasicDragonNet):
    """
